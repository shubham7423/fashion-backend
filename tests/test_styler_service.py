...
class TestStylerService:
    ...
    def test_get_user_json_file_path_with_subdirs(self):
        ...
        with patch("app.core.config.settings.CREATE_USER_SUBDIRS", True):
            with patch("app.core.config.settings.USER_DATA_DIRECTORY", "user_data"):
                with patch(
                    "app.core.config.settings.ATTRIBUTES_JSON_FILE", "attributes.json"
                ):
        ...
        user_data = {
            "images": {
                ...
                "hash3": {
                    ...
                    "attributes": {"identifier": "unknown", "category": "unknown"},
                },
            }
        }
    ...
    def test_validate_styling_parameters(self):
        ...
        params = StylerService.validate_styling_parameters(
            "New York", "cold weather", "business meeting"
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_outfit_recommendation_success(
        self, mock_gemini_styler, mock_load_user
    ):
        ...
        mock_styler_instance.style.return_value = (
            '{"top": "shirt.jpg", "bottom": "pants.jpg"}'
        )
