...
class StylerService:
    ...
    def load_user_attributes(user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error loading user data: {str(e)}"
            )

    @staticmethod
    def extract_clothing_attributes_for_styling(user_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for image_hash, image_data in images_data.items():
            ...
            if attributes and isinstance(attributes, dict):
                ...
                styling_item = {
                    "image": attributes.get("image", image_data.get("filename", f"image_{image_hash[:8]}")),
                ...
                if styling_item["identifier"] != "unknown" and styling_item["category"] != "unknown":
    ...
    async def generate_outfit_recommendation(
        ...
        try:
            ...
            clothing_attributes = StylerService.extract_clothing_attributes_for_styling(user_data)
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Styler initialization failed: {str(e)}"
                )
            ...
            try:
                ...
                outfit_recommendation = json.loads(outfit_json) if isinstance(outfit_json, str) else outfit_json
    ...
    @staticmethod
    def validate_styling_parameters(city: str, weather: str, occasion: str) -> Dict[str, str]:
        """Validate and sanitize styling parameters"""
        # Basic validation and sanitization
        validated_params = {
            "city": city.strip() if city else "Toronto",
            "weather": weather.strip() if weather else "early fall weather - expect temperatures around 15-20°C, partly cloudy",
